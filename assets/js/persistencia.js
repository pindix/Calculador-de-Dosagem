...
(function () {
    ...
    function salvarEstado() {
        ...
        if (typeof modoAtual !== 'undefined') {
            estado._modoAtual = modoAtual;
        }
        if (typeof estado?.intervaloAtual !== 'undefined') {
            estado._intervaloAtual = estado.intervaloAtual;
        }
        if (typeof isCicloAtivo !== 'undefined') {
            estado._isCicloAtivo = isCicloAtivo;
        }
        if (typeof isNaoSeiDum !== 'undefined') {
            estado._isNaoSeiDum = isNaoSeiDum;
        }
    ...
    function restaurarEstado() {
        ...
        // ===== 9. RESTAURA O SELECT DO ANTROPOMETRIA =====
        ...
        // ===== 11. RESTAURA O MODO DO GESTOGRAMA =====
        if (estado._modoAtual && typeof selecionarModo === 'function') {
            setTimeout(() => {
                try { selecionarModo(estado._modoAtual); } catch (e) { }
            }, 700);
        }
        ...
        // ===== 12. RESTAURA O RESULTADO =====
        if (estado._resultadoHTML) {
            setTimeout(() => {
                const resultado = document.getElementById('resultado');
                if (resultado) {
    ...
    // Salva quando clica em selects personalizados (Sinais Vitais)
    ...
    // Salva quando clica em selects de unidade (Fórmula Internacional)
    document.addEventListener('click', function (e) {
    ...
    // Salva quando clica em selects do Conversor
    document.addEventListener('click', function (e) {
        if (e.target.closest('.custom-select-option-unidade')) {
            setTimeout(salvarEstado, 150);
        }
    ...
    // Salva quando clica em selects do Gotejamento
    document.addEventListener('click', function (e) {
        if (e.target.closest('.custom-select-option-unidade')) {
            setTimeout(salvarEstado, 150);
        }
    ...
    // Salva quando clica no select do Antropometria
    document.addEventListener('click', function (e) {
    ...
    // Salva quando clica em selects da Diluição
    document.addEventListener('click', function (e) {
    ...
    // ===== NOVO: Salva quando clica em selects do Sistema ABO =====
    document.addEventListener('click', function (e) {
